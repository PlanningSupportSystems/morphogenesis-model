/*
 * 
 * MATER COMO REFERENCIA DE INICIO DE PROJETO
 * 
 * 
...
//    readonly ControleAglomeracao Controles = Terrain.activeTerrain.GetComponent<ControleAglomeracao>();
 ...
 //       this.predioPreFab = tipo;
    ...
    public void Start()
    {
        if (tracking)
        {
            Debug.Log("tracking Predios start");
        }
...
//    }
    ...
    public float CalcularAnguloVizinhos( List<Vector3> listaObjetos, float distanciaMaxima)
{
    if (tracking)
        {
...
//            if (objeto != objetoOriginal)
            if (objeto != posOriginal)
            {
                //float distancia = Vector3.Distance(posOriginal, objeto.transform.position);
                float distancia = Vector3.Distance(posOriginal, objeto);
...
//            Debug.Log("vizinho atual " + vizinho);
                ...
                vetorMedio += vizinho - posOriginal;
